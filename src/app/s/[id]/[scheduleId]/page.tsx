...
export default function StudentSchedulePage() {
  ...
  const handleApply = async (sid: string, selectedSlot?: SlotOption) => {
  ...
  if (!tenantId || !scheduleId) return null;

  if (schedules.length === 0 && serverTime === null) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-pastel-cream via-pastel-sky/20 to-pastel-mint/20 flex items-center justify-center p-4">
  ...
  if (schedules.length === 0) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-pastel-cream via-pastel-sky/20 to-pastel-mint/20 flex items-center justify-center p-4">
        <p className="text-gray-500">일정을 찾을 수 없어요.</p>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-pastel-cream via-pastel-sky/20 to-pastel-mint/20 relative">
      ...
      <div className="max-w-2xl mx-auto p-4 pb-12">
        <header className="text-center py-6">
          ...
          <p className="text-gray-600 text-sm mt-1">아래 항목을 입력한 뒤 신청하세요.</p>
        </header>
